...
def pega_produtos_amais(df, id_produto, tempo):
  ...
  codigo = produto["Codigo produto"].values[0][:-2] if "\t" in produto["Codigo produto"].values[0] else produto["Codigo produto"].values[0]
...
for id_produto in balanco_antes["ID Produto"]:
  ...
  if (qnt_antes != qnt_depois):
    ...
    codigo = balanco_antes.loc[balanco_antes["ID Produto"] == id_produto]["Codigo produto"].values[0][:-2] if "\t" in balanco_antes.loc[balanco_antes["ID Produto"] == id_produto]["Codigo produto"].values[0] else balanco_antes.loc[balanco_antes["ID Produto"] == id_produto]["Codigo produto"].values[0]
    df_produto = pd.DataFrame({
      "C�digo": codigo,
